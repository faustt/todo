...
<form on:submit|preventDefault={register} class="m-auto mt-24 bg-white shadow-lg rounded-md max-w-sm p-4">
    <div class="flex flex-col gap-4">
        <div class="flex flex-row items-center">
            <div class="w-28 select-none">Name</div>
            <div class="flex-1 relative h-10">
                <input disabled={isWorking} type="text" class="input" bind:value={name} />
            </div>
        </div>
        <div class="flex flex-row items-center">
            <div class="w-28 select-none">Password</div>
            <div class="flex-1 relative h-10">
                <input disabled={isWorking} type="password" class="input" bind:value={password} />
            </div>
        </div>
        <button disabled={isWorking} class="button" type="submit">Register</button>
    </div>
</form>
