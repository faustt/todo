...
export default {
            ...
            if (todoToMove.position < todoToSwapWith.position) {
                ...
                const todosToMove = await trans.db
                    ...
                    .between(
                        ...
                        [todoToMove.categoryId, todoToSwapWith.position + (swap === "before" ? -1 : 0)],
                        true,
                        true,
                    )
                    .toArray();
                ...
                const todosToMove = await trans.db
                    ...
                    .where(["categoryId", "position"])
                    .between(
                        [todoToMove.categoryId, todoToSwapWith.position + (swap === "before" ? 0 : 1)],
                        [todoToMove.categoryId, todoToMove.position],
                        true,
                        true,
                    )
    ...
    async "category/position-changed"(event) {
        ...
        await db.transaction("readwrite", "categories", async (trans) => {
            ...
            const categoryToMove = await trans.db.table("categories").where({ id: event.payload.id }).first();
            if (categoryToMove.id === beforeId || categoryToMove.id === afterId) {
                return;
            }
            ...
            if (event.payload.beforeId) {
                console.debug("BEFORE");
                categoryToSwapWith = await trans.db.table("categories").where({ id: beforeId }).first();
                swap = "before";
            } else {
                console.debug("AFTER");
                categoryToSwapWith = await trans.db.table("categories").where({ id: afterId }).first();
                swap = "after";
            }
            ...
            if (categoryToMove.position < categoryToSwapWith.position) {
                await trans.db.table("categories").where({ id: categoryToMove.id }).delete();
                const categoriesToMove = await trans.db
                    .table("categories")
                    .where(["userId", "position"])
                    .between(
                        [categoryToMove.userId, categoryToMove.position],
                        [categoryToMove.userId, categoryToSwapWith.position + (swap === "before" ? -1 : 0)],
                        true,
                        true,
                    )
                    .toArray();
                for (const category of categoriesToMove) {
                    await trans.db.table("categories").where({ id: category.id }).delete();
                    category.position--;
                ...
                console.debug(categoriesToMove.map((cat) => ({ name: cat.name, position: cat.position })));
                ...
                const categoriesToMove = await trans.db
                    ...
                    .where(["userId", "position"])
                    .between(
                        [categoryToMove.userId, categoryToSwapWith.position + (swap === "before" ? 0 : 1)],
                        [categoryToMove.userId, categoryToMove.position],
                        true,
                        true,
                    )
                ...
                categoryToMove.position = categoryToSwapWith.position + (swap === "before" ? 0 : 1);
                await trans.db.table("categories").add(categoryToMove);
                console.debug(categoriesToMove.map((cat) => ({ name: cat.name, position: cat.position })));
            }
        });
