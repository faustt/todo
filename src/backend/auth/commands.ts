...
export default {
    async createUser(args: CreateUserArgs) {
        if (typeof args.name !== "string" || typeof args.password !== "string") {
            throw new Error("Invalid data!");
        }

        if (args.name.length < 2) {
            throw new Error("User name too short. Must be at least 2 characters long!");
        }

        if (args.name.length > 32) {
            throw new Error("User name too long. Must be at most 32 characters long!");
        }

        if (args.password.length < 5) {
            throw new Error("Password too short. Must be at least 5 characters long!");
        }

        if (args.password.length > 64) {
            throw new Error("User name too long. Must be at most 64 characters long!");
        }
    ...
    async login(args: LoginArgs) {
        ...
        const existingUser = (await this.getUserWith({ name: args.name })) as User;
        if (!existingUser) {
            fail();
        }
